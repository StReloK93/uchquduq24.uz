    ...
    <section @mousedown="close" class="absolute inset-0 bg-black/50 z-50">
        ...
        >
            ...
            <div class="mb-4">
                ...
                <input type="text" v-model="formData.name" class="text-input w-full">
            </div>
            <div class="mb-4">
                <label for="name" class="text-gray-500 text-sm">Kiritilgan sana</label>
                <input type="date" v-model="formData.created_at" class="text-input w-full">
            </div>
...
function addPost(){
    ...
    axios.post('posts', Form).then(({data}) => {
        emit('add')
        close()
    })
}
